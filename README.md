...
Hi community,
I commit to this repository to experiment the domain of AI agents, to know what all they can do, and get any fruitful outcomes if possible.
...
We have been exposed to artificial intelligence for years, and are surrounded by products like Amazon Echo, Google Voice Assistant, which have been working on closed source AI models for a long time.

After the launch of OpenAI's ChatGPT, access to these models went public. So now, anyone can make their own Assistant in their premise, providing them exclusive information like account credentials, access to social handles, credit card information, which people otherwise won't have provided to Alexa, or Gemini, because they executed on alien platforms. And hence, anyone can have a service that can do anything in their premise and supervision.

But there are many limitations yet.

These public LLMs can only do small units of work. You need to build a React component? Tell the dumb LLM directly and it will do it for you. But we don't want to tell them what to do every now and then. We want to create **workflows** these dumb LLMs can use, and get large units of work done **without** human intervention. 
...
So we can create services that guides these dumb LLMs to understand what we want directly want them to understand.
